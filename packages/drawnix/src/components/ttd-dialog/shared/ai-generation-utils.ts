...
const getPromptOptimizationLabels = (
  type: PromptOptimizationRequestOptions['type'],
  language: 'zh' | 'en'
) => {
  if (language === 'zh') {
    switch (type) {
      case 'image':
        return {
          modalityLabel: '图片生成',
          domainFocus: '重点补足主体、构图、风格、光线、材质与画面细节。',
        };
      case 'video':
        return {
          modalityLabel: '视频生成',
          domainFocus: '重点补足主体动作、镜头语言、运镜、时序、转场与画面连续性。',
        };
      case 'audio':
        return {
          modalityLabel: '音频生成',
          domainFocus: '重点补足风格、节奏、情绪、乐器/人声、歌词结构与使用场景。',
        };
      case 'text':
        return {
          modalityLabel: '文本生成',
          domainFocus: '重点补足主题、结构、受众、语气、信息密度与输出格式。',
        };
      case 'agent':
        return {
          modalityLabel: 'Agent 指令',
          domainFocus: '重点补足目标、上下文、约束、输入输出、执行步骤与验收标准。',
        };
      default:
        return {
          modalityLabel: '生成',
          domainFocus: '重点补足目标、上下文、约束与输出要求。',
        };
    }
  }
  ...
  switch (type) {
    case 'image':
      return {
        modalityLabel: 'image generation',
        domainFocus:
          'Improve subject detail, composition, style, lighting, materials, and visual detail.',
      };
    case 'video':
      return {
        modalityLabel: 'video generation',
        domainFocus:
          'Improve subject motion, camera language, camera movement, timing, transitions, and continuity.',
      };
    case 'audio':
      return {
        modalityLabel: 'audio generation',
        domainFocus:
          'Improve style, rhythm, mood, instruments or vocals, lyrical structure, and use case.',
      };
    case 'text':
      return {
        modalityLabel: 'text generation',
        domainFocus:
          'Improve topic, structure, audience, tone, information density, and output format.',
      };
    case 'agent':
      return {
        modalityLabel: 'agent instruction',
        domainFocus:
          'Improve goal, context, constraints, inputs, outputs, execution steps, and acceptance criteria.',
      };
    default:
      return {
        modalityLabel: 'generation',
        domainFocus: 'Improve goals, context, constraints, and output requirements.',
      };
  }
};
...
export const buildPromptOptimizationRequest = ({
  ...
  const { modalityLabel, domainFocus } = getPromptOptimizationLabels(
    type,
    language
  );
  const isVisualGeneration = type === 'image' || type === 'video';
  ...
  if (isStructuredMode) {
    if (language === 'zh') {
      return [
        ...
        isVisualGeneration
          ...
          : '请把原始需求整理成稳定、可复用、可继续编辑的 JSON 结构化提示词。',
        '',
        '目标：',
        '1. 保留用户原始意图，不要擅自改题。',
        isVisualGeneration
          ? '2. 将复杂场景拆解为稳定、可复用、可继续编辑的结构字段。'
          : '2. 将目标、上下文、约束、步骤和输出拆解为清晰的结构字段。',
        isVisualGeneration
          ? '3. 如果原文中有明确的布局、数量、标题、区域、主体、材质、风格、时间线、图例、标注等信息，尽量显式结构化。'
          : '3. 如果原文中有明确的受众、风格、格式、输入材料、验收标准或限制条件，尽量显式结构化。',
        `4. ${domainFocus}`,
        ...
        isVisualGeneration
          ...
          : '1. 顶层优先包含这些键：type、instruction、context、constraints、output。',
        isVisualGeneration
          ? '2. style 是对象，放背景、主体风格、材质、镜头或质感等。'
          : '2. context 是对象，放目标、受众、输入材料、背景信息和使用场景等。',
        isVisualGeneration
          ? '3. layout 是对象，放标题、分区、数量、位置、图例、时间轴、主体布局等。'
          : '3. constraints 是对象，放语气、格式、长度、禁用项、模型偏好和执行边界等。',
        isVisualGeneration
          ? '4. 对复杂主体区域，优先使用 centerpiece、focal_point、sections、annotations、timeline、legends 等语义化字段。'
          : '4. output 是对象，放最终产物类型、结构、字段、质量标准和验收条件等。',
        ...
        trimmedRequirements ||
          (isVisualGeneration
            ...
            : '将其整理成清晰、可执行、可继续扩写与复用的 JSON 提示词。'),
      ].join('\n');
    }
    ...
    return [
      ...
      isVisualGeneration
        ...
        : 'Transform the original request into a stable, reusable, and editable JSON-based structured prompt.',
      '',
      'Goals:',
      ...
      isVisualGeneration
        ...
        : '2. Decompose goals, context, constraints, steps, and outputs into clear fields.',
      isVisualGeneration
        ...
        : '3. Explicitly structure audience, style, format, source inputs, acceptance criteria, and constraints whenever present.',
      `4. ${domainFocus}`,
      ...
      isVisualGeneration
        ...
        : '1. Prefer these top-level keys: type, instruction, context, constraints, output.',
      isVisualGeneration
        ? '2. style should be an object for background, subject style, materials, camera, rendering, or texture.'
        : '2. context should be an object for goals, audience, source inputs, background, and use case.',
      isVisualGeneration
        ? '3. layout should be an object for title, sections, counts, positions, legends, timelines, and composition.'
        : '3. constraints should be an object for tone, format, length, exclusions, model preferences, and execution boundaries.',
      isVisualGeneration
        ...
        : '4. output should be an object for deliverable type, structure, fields, quality bar, and acceptance criteria.',
      ...
      trimmedRequirements ||
        (isVisualGeneration
          ...
          : 'Restructure it into a clear, executable, reusable JSON prompt.'),
    ].join('\n');
  }
