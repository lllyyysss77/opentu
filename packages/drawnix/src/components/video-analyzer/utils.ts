...
6. 若提供画面风格，video_style、description、first_frame_prompt、非空 last_frame_prompt 必须共同继承该风格，不要只写在总字段里；全局风格只能作为背景约束，不能覆盖每个镜头自己的核心动作和构图差异。
...
11. **suggestion（生成建议）**：必须根据改编后的最终脚本重写，用于第3步首帧/视频生成；只写稳定的风格、角色、视觉锚点、镜头衔接、片段差异化重点、生成禁忌和执行策略，不要复述“用户提示词/改编指令”本身
