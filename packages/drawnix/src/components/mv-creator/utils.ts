...
4. first_frame_prompt 中若有角色，必须包含对应角色的完整外貌描述，并明确沿用同一人物身份、脸型五官、发型、体型、年龄感、完整服装款式、服装颜色、材质和配饰，不得重新设计衣服；last_frame_prompt 可为空，只有确实需要独立尾帧图时才填写，非空时必须保持同一人物、同一发型、同一套衣服和同一配饰
...
- first_frame_prompt: 首帧图片提示词（精确描述主体位置、动作起始状态、构图、光线与背景；若该镜头有角色，必须包含对应角色的完整外貌、发型、脸部特征、体型、年龄感、服装款式、服装颜色、材质和配饰）
- last_frame_prompt: 尾帧图片提示词，可为空字符串（只有需要独立尾帧图时才填写；若下一段首帧自然承接为本段尾帧，则填 ""；非空时必须沿用同一人物和同一套服装，只改变结尾姿态、表情、动作定格、构图、光线或背景）
...
3. first_frame_prompt（首帧图片提示词）：精确描述主体位置、动作起始状态、构图、光线与背景${hasCharacters ? '；若该镜头有角色，必须包含对应角色的完整外貌描述，并明确沿用同一人物身份、脸型五官、发型、体型、年龄感、完整服装款式、服装颜色、材质和配饰，不得重新设计衣服' : ''}
